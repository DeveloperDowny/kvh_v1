...
class BlockController {
    ...
    test = async (req, res) => {
        ...
        try {
            ...
            // https://services.tokenview.io/vipapi/{lowercase-coin-abbr}/address/{address}?apikey={apikey}
                ...
                let data = await axios.get(
                    `https://services.tokenview.io/vipapi/${nw}/address/${id.toLowerCase()}?apikey=${process.env.vTOKEN
                    }`
                );

                let firstTx = await axios.get(
                    `https://services.tokenview.io/vipapi/firsttx/${nw}/${id.toLowerCase()}?apikey=${process.env.vTOKEN
                    }`
                );
    ...
    getRisk = async (req, res) => {
        try {
            ...
            let riskMessage = getRiskMessage(parseInt(jsonResponse.riskScores.combinedRisk))
